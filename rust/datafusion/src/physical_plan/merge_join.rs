...
use crate::physical_plan::common::create_batch_empty;
...
impl MergeJoinStreamState {
    ...
    pub fn empty_batch(&self) -> ArrowResult<RecordBatch> {
        create_batch_empty(self.stream.schema().as_ref())
    }
}
...
pub fn concat_batches(
    ...
    assert!(batches.len() > 0, "Empty batch provided");
    if batches.len() == 1 {
    ...
    let columns = (0..batches[0].1.num_columns())
        .map(|column_index| -> ArrowResult<ArrayRef> {
            ...
            let concat_array = concat(columns.as_slice())?;
            assert_eq!(
                concat_array.len(),
...
fn merge_join(
    ...
    let columns = schema
        ...
        .map(|field| {
            left.schema()
                .index_of(field.name())
                .and_then(|i| take(left.column(i), left_indices.as_ref(), None))
                .or_else(|_| {
                    right
                        .schema()
                        .index_of(field.name())
                        .and_then(|i| take(right.column(i), right_indices.as_ref(), None))
                })
        })
        .collect::<ArrowResult<Vec<_>>>()?;
...
mod tests {
    ...
    async fn all_nulls_multiple() -> Result<()> {
        ...
        let result = format_batch(&batches[1]);
        let expected = vec!["NULL,NULL,NULL,NULL,NULL", "NULL,NULL,NULL,NULL,NULL"];
        assert_same_rows(&result, &expected);
    ...
    async fn multiple_remainders() -> Result<()> {
        ...
        let result = format_batch(&batches[3]);
        let expected = vec![];
        assert_same_rows(&result, &expected);

        let result = format_batch(&batches[4]);
        let expected = vec![
            "2,5,8,11,111",
            "2,5,8,12,112",
            "2,5,8,21,121",
        ...
        let result = format_batch(&batches[5]);
        let expected = vec!["3,6,9,52,152"];
        assert_same_rows(&result, &expected);
